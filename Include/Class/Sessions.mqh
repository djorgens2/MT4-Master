...
//+------------------------------------------------------------------+
//| Sessions Class - Collects session data, states, and events       |
//+------------------------------------------------------------------+
class CSessions
  {
...
public:
             ...
             CSessions(SessionType Type, int HourOpen, int HourClose);
            ~CSessions();

             SessionType   Type(void)            {return (sType);}
             bool          IsOpen(void);
             CEvent       *Event(int Type=Global);
             bool          ActiveEvent(void)     {return (sEventGlobal.ActiveEvent());}
...
private:
             ...
             void          SetTermState(EventType Event);
...
//+------------------------------------------------------------------+
//| SetTermState - Update term state and set market correction price |
//+------------------------------------------------------------------+
void CSessions::SetTermState(EventType Event)
  {
    ReservedWords  stsLastTermState  = srActive.State;
    
    switch (Event)
    {
      case NewBreakout:  if (srActive.BreakoutDir==DirectionNone)
                         {
                           srActive.State           = Breakout;
                           srActive.CorrectionPrice = srActive.OffMid;
                           srActive.BreakoutDir     = BoolToInt(sEventTerm[NewHigh],DirectionUp,DirectionDown);
                           
                           sEventTerm.SetEvent(NewBreakout);
                         }

                         if (sEventTerm[NewHigh])
                           if (IsChanged(srActive.BreakoutDir,DirectionUp))
                           {
                             srActive.State          = Reversal;
                                                          
                             sEventTerm.SetEvent(NewReversal);
                             sEventTerm.SetEvent(NewState);
                           }
                           
                         if (sEventTerm[NewLow])
                           if (IsChanged(srActive.BreakoutDir,DirectionDown))
                           {
                             srActive.State          = Reversal;
    ...
    if (IsChanged(stsLastTermState,srActive.State))
      sEventTerm.SetEvent(NewState);
  }
...
//+------------------------------------------------------------------+
void CSessions::SetTrendState(ReservedWords State)
  {
    srTrend.State           = State;
...
//+------------------------------------------------------------------+
void CSessions::OpenSession(void)
  {    
    sEventGlobal.SetEvent(SessionOpen);
...
//+------------------------------------------------------------------+
void CSessions::CloseSession(void)
  {
    ...
    ArrayCopy(srHistory,uhHistory,1);
      ...
      {
        sEventGlobal.SetEvent(SessionClose);
...
//      
    srActive.BreakoutDir      = DirectionNone;
...
//+------------------------------------------------------------------+
//| UpdateSession - Updates active pricing and sets events           |
//+------------------------------------------------------------------+
void CSessions::UpdateSession(void)
  {
    //--- Calc boundary events
    {
      ...
      {
        sEventTerm.SetEvent(NewHigh);
        sEventTerm.SetEvent(NewBoundary);

        if (IsChanged(srActive.TermDir,DirectionUp))
          sEventTerm.SetEvent(NewDirection);
      }
        
      //--- Test for session low
      if (IsLower(Low[sBar],srActive.TermLow))
      {
        sEventTerm.SetEvent(NewLow);
        sEventTerm.SetEvent(NewBoundary);
      ...
      if (sEventTerm[NewBoundary])
      {
        sEventGlobal.SetEvent(NewTerm);

        if (IsHigher(High[sBar],srActive.Resistance) || IsLower(Low[sBar],srActive.Support))
          SetTermState(NewBreakout);
      }       
      else
...
//+------------------------------------------------------------------+
void CSessions::CalcEvents(void)
  {
    //--- Clear events
    sEventGlobal.ClearEvents();
    sEventTerm.ClearEvents();
    sEventTrend.ClearEvents();
    ...
    {      
      ...
      sEventGlobal.ClearEvents();
      sEventGlobal.SetEvent(NewDay);
    }
    
    if (IsChanged(sBarHour,TimeHour(Time[sBar])))
      sEventGlobal.SetEvent(NewHour);
...
//+------------------------------------------------------------------+
void CSessions::UpdateBuffers(void)
  {
    if (Bars<sBars)
...
//+------------------------------------------------------------------+
CSessions::CSessions(SessionType Type, int HourOpen, int HourClose)
  {
    ...
    sEventGlobal                 = new CEvent();
    sEventTerm                   = new CEvent();
    sEventTrend                  = new CEvent();
...
//+------------------------------------------------------------------+
void CSessions::Update(double &OffMidBuffer[], double &PriorMidBuffer[])
  {
    Update();
...
//+------------------------------------------------------------------+
double CSessions::ActiveMid(void)
  {
...
//+------------------------------------------------------------------+
int CSessions::Direction(RetraceType Type)
  {
    if (Type==Trend)
...
//+------------------------------------------------------------------+
int CSessions::SessionHour(int Measure=Now)
  {    
    switch (Measure)
    {
...
//+------------------------------------------------------------------+
//| Event - Returns the event record for the specified type          |
//+------------------------------------------------------------------+
CEvent *CSessions::Event(int Type)
  {    
    switch (Type)
    {
      case Trend:      return(sEventTrend);
      case Term:       return(sEventTerm);
      case Global:     return(sEventGlobal);
    }
...
//+------------------------------------------------------------------+
ReservedWords CSessions::State(RetraceType Type)
  {    
    switch (Type)
    {
