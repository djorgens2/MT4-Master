...
public:
            ...
            ~CSessionArray();
             ...
             SessionRec    Active(int Shift)         {return (srOpenSession[Shift]);}
             SessionRec    OffSession(int Shift)     {return (srOffSession[Shift]);}
...
private:
             ...
             SessionRec    srOpenSession[];
             SessionRec    srOffSession[];
             
             
             CArrayDouble *sOffSessionBuffer;
             CArrayDouble *sOpenSessionBuffer;
...
void CSessionArray::OpenSession(void)
  {    
    ...
    sOpenSessionBuffer.SetValue(sBar,srActive.OffMid);
    sOpenSessionBuffer.SetValue(sBar,srActive.PriorMid);
  }
...
void CSessionArray::UpdateBuffers(void)
  {
    if (Bars<sBars)
      ...
      {
        sOpenSessionBuffer.Insert(0,0.00);
        sOpenSessionBuffer.Insert(0,0.00);
      }
  }
...
CSessionArray::CSessionArray(SessionType Type, int HourOpen, int HourClose)
  {
    ...
    sOffSessionBuffer               = new CArrayDouble(Bars);
    sOffSessionBuffer.Truncate      = false;
    sOffSessionBuffer.AutoExpand    = true;    
    sOffSessionBuffer.SetPrecision(Digits);
    sOffSessionBuffer.Initialize(0.00);
    
    sOpenSessionBuffer              = new CArrayDouble(Bars);
    sOpenSessionBuffer.Truncate     = false;
    sOpenSessionBuffer.AutoExpand   = true;
    sOpenSessionBuffer.SetPrecision(Digits);
    sOpenSessionBuffer.Initialize(0.00);
...
//+------------------------------------------------------------------+
void CSessionArray::Update(double &OffSessionBuffer[], double &OpenSessionBuffer[])
  {
    Update();
    
    sOpenSessionBuffer.Copy(OffSessionBuffer);
    sOpenSessionBuffer.Copy(OpenSessionBuffer);
  }
